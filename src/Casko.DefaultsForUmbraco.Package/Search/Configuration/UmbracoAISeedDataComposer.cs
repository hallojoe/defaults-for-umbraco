...
{
    private static readonly JsonSerializerOptions JsonOptions = Umbraco.AI.Core.Constants.DefaultJsonSerializerOptions;

    public const string DefaultConnectionAlias = "openai-demo";
    ...
    public async Task HandleAsync(UmbracoApplicationStartedNotification notification, CancellationToken ct)
    {
        ...
        if (await connectionService.GetConnectionByAliasAsync(DefaultConnectionAlias, ct) is not null)
        {
